...
function FormTotalWaterYield() {
  ...
  const { data: areas } = useQuery(getRegion);
        ...
        <Col xs={24} sm={12} md={12} lg={12}>
          <Form.Item name="vung" label="Vùng: ">
            <Select
              fieldNames="vung"
              options={
                areas?.GetVungs?.nodes?.length <= 0
                  ? []
                  : areas?.GetVungs?.nodes?.map((_nameManager) => ({
...
export default FormTotalWaterYield;
