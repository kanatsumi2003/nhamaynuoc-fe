...
import { fetchReadingIndexTotalFilterBlock } from "../../redux/slices/readingIndexTotalSlice/readingIndexTotalSlice";
import readingIndexTotalSlice from "../../redux/slices/readingIndexTotalSlice/readingIndexTotalSlice";

export const FormSearchWriteModalIndexBlock = ({
  listKGCS,
  listTuyenDoc,
  listKhuVuc,
  canBoDocs,
  ...
  const onFinish = (values) => {
    if (values) {
      dispatch(fetchReadingIndexTotalFilterBlock({ ...values, nhaMayId }))
      dispatch(
        readingIndexTotalSlice.actions.btnClickFilterBlock({
          ...values,
          nhaMayId,
          pageNumber: 1,
        })
